...
public class MscDLL : MonoBehaviour {
    ...
    [DllImport("msc_x64", CallingConvention = CallingConvention.StdCall)]
    public static extern int QISRAudioWrite(IntPtr sessionID, byte[] wavaData, uint waveLen, AudioStatus autdioStatus, ref EpStatus epStatus, ref RecogStatus recogStatus);

    [DllImport("msc_x64", CallingConvention = CallingConvention.StdCall)]
    public static extern IntPtr QISRGetResult(IntPtr sessionID, ref RecogStatus reltStatus, int waitTime, ref int errorCode);
